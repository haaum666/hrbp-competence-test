...
const prepareDataForExport = (testResult: TestResult) => {
  ...
  data.push({
    ...
    'Категория ID': 'Категория ID', // ИСПРАВЛЕНО
    'Уровень сложности': 'Уровень сложности', // ИСПРАВЛЕНО
    'Время на вопрос (сек, оценка)': 'Время на вопрос (сек, оценка)', // Обновил название для ясности
    ...
    'Время потрачено (сек, факт)': 'Время потрачено (сек, факт)', // Обновил название для ясности
  });
  ...
  testResult.answers.forEach((answerDetail: AnswerDetail) => {
    ...
    data.push({
      ...
      'Категория ID': question.categoryid ?? 'Не указано', // ИСПРАВЛЕНО
      'Уровень сложности': question.level ?? 'Не указано', // ИСПРАВЛЕНО
      'Время на вопрос (сек, оценка)': question.timeEstimate,
      ...
      'Время потрачено (сек, факт)': userAnswer?.timeSpent?.toFixed(0) || '0',
    });
  });
