...
const TestPage: React.FC = () => {
  ...
  return (
    // <-- ИЗМЕНЕНИЯ ЗДЕСЬ: убираем большинство классов из корневого div
    // Теперь App.tsx main отвечает за min-height и padding
    // Этот div будет просто оберткой, которая растягивается по ширине
    <div className="w-full h-full flex flex-col items-center justify-start px-4 sm:px-0"> {/* <-- ИЗМЕНЕНИЯ ТУТ */}
      ...
      {testStarted && questions.length > 0 && !testFinished && (
        // В QuestionRenderer уже есть свои стили, здесь просто обеспечиваем его отображение
        <QuestionRenderer
